...
locals {
  ...
  names = {
    ...
    private_endpoint     = "pep-${var.workload}-${var.environment}-${local.loc}-blob"
    private_dns_zone     = "privatelink.blob.core.windows.net"
    dns_zone_vnet_link   = "vnetlnk-${var.workload}-${var.environment}-${local.loc}-blob"
    dns_zone_group       = "pdnszg-${var.workload}-${var.environment}-${local.loc}-blob"
  }
