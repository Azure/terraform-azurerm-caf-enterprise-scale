...
echo "==> Azure Root ID 1 - $TF_ROOT_ID_1"
echo "==> Azure Root ID 1 - $TF_ROOT_ID_2"
echo "==> Azure Root ID 1 - $TF_ROOT_ID_3"

#echo "==> Update root-id values..."
#id_1=$TF_ROOT_ID_1 id_2=$TF_ROOT_ID_2 id_3=$TF_ROOT_ID_3 yq eval '(.child_modules[].resources[] |= select(.address == "*root_id_1*").values.name = strenv(id_1)) | (.child_modules[].resources[] |= select(.address == "*root_id_2*").values.name = strenv(id_2))| (.child_modules[].resources[] |= select(.address == "*root_id_3*").values.name = strenv(id_3))' -i "$TF_PLAN_JSON"_planned_values.yml
