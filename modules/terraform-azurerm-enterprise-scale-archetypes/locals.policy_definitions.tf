...
locals {
  builtin_policy_definitions_from_json = local.archetype_policy_definitions_specified ? tolist(fileset(local.builtin_library_path, "**policy_definition_*.json")) : null
  builtin_policy_definitions_from_yaml = local.archetype_policy_definitions_specified ? tolist(fileset(local.builtin_library_path, "**policy_definition_*.{yml,yaml}")) : null
  custom_policy_definitions_from_json  = local.archetype_policy_definitions_specified && local.custom_library_path_specified ? tolist(fileset(local.custom_library_path, "**policy_definition_*.json")) : null
  custom_policy_definitions_from_yaml  = local.archetype_policy_definitions_specified && local.custom_library_path_specified ? tolist(fileset(local.custom_library_path, "**policy_definition_*.{yml,yaml}")) : null
}
...
locals {
  builtin_policy_definitions_dataset_from_json = try(length(local.builtin_policy_definitions_from_json) > 0, false) ? {
    ...
    filepath => jsondecode(file("${local.builtin_library_path}/${filepath}"))
  } : null
  builtin_policy_definitions_dataset_from_yaml = try(length(local.builtin_policy_definitions_from_yaml) > 0, false) ? {
    ...
    filepath => yamldecode(file("${local.builtin_library_path}/${filepath}"))
  } : null
  custom_policy_definitions_dataset_from_json = try(length(local.custom_policy_definitions_from_json) > 0, false) ? {
    ...
    filepath => jsondecode(file("${local.custom_library_path}/${filepath}"))
  } : null
  custom_policy_definitions_dataset_from_yaml = try(length(local.custom_policy_definitions_from_yaml) > 0, false) ? {
    ...
    filepath => yamldecode(file("${local.custom_library_path}/${filepath}"))
  } : null
}
